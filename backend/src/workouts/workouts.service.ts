...
export class WorkoutsService {
  ...
  async update(
    ...
    const existingWorkout = await this.workoutModel
      .findOneAndUpdate({ _id: id, user: userId }, updateWorkoutDto, {
        new: true,
      })
      .exec();
    if (!existingWorkout) {
  ...
  async addExerciseToWorkout(
    ...
    if (!updatedWorkout) {
      throw new NotFoundException(
        `Workout #${workoutId} not found or you do not have permission to modify it`,
      );
    }
  ...
  async updateExerciseInWorkout(
    ...
    updateData: { sets?: number; reps?: number; weight?: number },
  ): Promise<Workout> {
    if (
      !Types.ObjectId.isValid(workoutId) ||
      !Types.ObjectId.isValid(exerciseObjId)
    ) {
    ...
    if (updateData.sets !== undefined)
      updateFields['exercises.$.sets'] = updateData.sets;
    if (updateData.reps !== undefined)
      updateFields['exercises.$.reps'] = updateData.reps;
    if (updateData.weight !== undefined)
      updateFields['exercises.$.weight'] = updateData.weight;
    ...
    if (!updatedWorkout) {
      throw new NotFoundException(
        `Workout #${workoutId} or specific exercise not found, or you lack permission to modify it`,
      );
    }
  ...
  async removeExerciseFromWorkout(
    ...
    const updatedWorkout = await this.workoutModel
      .findOneAndUpdate(
        { _id: workoutId, user: userId },
        { $pull: { exercises: { _id: exerciseObjId } } as any },
        { new: true },
      )
      .exec();
    ...
    if (!updatedWorkout) {
      throw new NotFoundException(
        `Workout #${workoutId} not found or you lack permission to modify it`,
      );
    }
  ...
  async getAiRecommendation(userId: string, workoutId: string) {
    ...
    const exercisePromises = workout.exercises.map(async (ex) => {
      try {
        const exDetails = await this.exercisesService.getExerciseById(
          ex.exerciseId,
        );
    ...
    try {
          ...
          responseSchema: {
            ...
            properties: {
              isSafe: {
                ...
                description:
                  'True if generally safe, False if heavily contraindicated by their illnesses',
              },
              warnings: {
                type: SchemaType.ARRAY,
                items: { type: SchemaType.STRING },
                description:
                  'Specific warnings about exercise selection correlated to their illnesses or goals',
              },
              positiveFeedback: {
                type: SchemaType.ARRAY,
                items: { type: SchemaType.STRING },
              ...
              generalAdvice: {
                ...
                description:
                  'A highly personalized 2-sentence summary/recommendation.',
              },
              exerciseAssessments: {
                type: SchemaType.ARRAY,
                items: {
                  ...
                  properties: {
                    exerciseId: {
                      type: SchemaType.STRING,
                      description:
                        'The exact ID string of the exercise from the prompt',
                    },
                    status: {
                      type: SchemaType.STRING,
                      description:
                        "Must be exactly one of: 'Recommended', 'Not Recommended', 'Consult Doctor', 'Neutral'",
                    },
                    reason: {
                      type: SchemaType.STRING,
                      description:
                        'Brief 1-sentence reason for this specific recommendation based on their profile',
                    },
                  },
                  required: ['exerciseId', 'status', 'reason'],
                },
                description:
                  'An assessment array containing an evaluation for EVERY SINGLE exercise ID provided in the WORKOUT list.',
              },
            },
            required: [
              'isSafe',
      ...
      let rawJsonStr = result.response.text();

      console.log('=== GEMINI RAW JSON OUTPUT ===');
      console.log(rawJsonStr);
      console.log('==============================');

      // Safety measure: Strip out any markdown blocks if the AI hallucinates them
      ...
      if (error?.status === 429 || error?.message?.includes('429')) {
        return {
          isSafe: true,
          warnings: [
            'We are currently experiencing high demand and our AI coach is taking a breather. Please try again in about 30 seconds.',
          ],
          positiveFeedback: ['Your workout is saved and ready!'],
          generalAdvice:
            'Gemini AI is currently handling too many requests. Please wait a moment and refresh for your full safety analysis.',
          exerciseAssessments: [],
        };
      }

      throw new InternalServerErrorException(
        'Failed to generate AI workout analysis.',
      );
    }
  }
}